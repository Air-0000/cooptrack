...
# UACP: Uncertainty-Aware Cooperative Perception settings
is_cooperation = True
learn_match = True
veh_thre = 0.4

# UACP module flags
use_caa = True      # Context-Aware Association (component of UGIM)
use_aaf = True       # Altitude-Adaptive Fusion (component of DGC)
use_emb = False      # Cross-View Embedding (OFF by default — ablation only; saves ~4ms)
use_comm = False     # Communication Uncertainty (OFF by default — ablation only)
h_ref = 25.0         # Reference altitude for drone (meters)
training_stage = 3   # 1=vehicle-only, 2=infrastructure-only, 3=cooperative
use_gru = False      # Dual GRU (OFF by default — ablation only; enabled via training_stage)
...
train_pipeline = [
    ...
    dict(
        ...
        meta_keys=(
            ...
            'scale_factor', 'flip', 'pcd_horizontal_flip', 'pcd_vertical_flip', 'box_mode_3d', 'box_type_3d',
            'img_norm_cfg', 'pcd_trans', 'sample_idx', 'sample_idx_inf', 'prev_idx', 'next_idx',
            'pcd_scale_factor', 'pcd_rotation', 'pts_filename', 'transformation_3d_flow', 'scene_token', 'can_bus',
        ),
    ),
]
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            dict(type='RandomScaleImageMultiViewImage', scales=[0.5]),
            dict(type='PadMultiViewImage', size_divisor=32),
            dict(type="CustomDefaultFormatBundle3D", class_names=class_names, with_label=False, collect_keys=inf_keys),
            dict(
                type="CustomCollect3D",
                keys=[
                    "veh2inf_rt",
...
# Runner and evaluation settings
