...
# Infrastructure-side point cloud range (wider on +x to cover the larger drone FOV)
...
# Training settings (infrastructure-only single-end)
train_gt_iou_threshold = 0.3

# Save track queries for downstream cooperative training (Stage 3)
...
# UACP: stage 2 (infrastructure-only single-end training)
# All UACP cooperation modules are disabled at this stage.
is_cooperation = False
learn_match = False
veh_thre = 0.4

# UACP module flags (all OFF for single-end training)
use_caa = False
use_aaf = False
use_emb = False
use_comm = False
h_ref = 25.0
training_stage = 2   # 2=infrastructure-only
use_gru = False

# Query keys that Stage 3 cooperative training reads back from saved files.
...
model = dict(
    ...
    spatial_temporal_reason=dict(
        ...
        fut_len=8,
        pc_range=point_cloud_range,
        # UACP flags are unused at this stage (no cross-agent interaction),
        # but we pass them so configs share a uniform schema across stages.
        cross_agent_interaction=dict(
            ...
            h_ref=h_ref,
            pc_range=point_cloud_range,
            inf_pc_range=point_cloud_range,
            embed_dims=_dim_,
        ),
...
# UACP / Adaptive Fusion stage marker (read by tools that branch on stage).
_uacp_stage_ = 2
_uacp_stage_name_ = "infrastructure-only"