...
## 七、论文 ↔ 代码一致性对齐（2026-09-22）

对 857 行稿件与实际代码逐条比对，修复 6 处实质性错位。除最后一处外均为代码改动，
论文侧只修正自身矛盾与占位数字。

| # | 问题 | 修复 |
|---|---|---|
| 1 | 主管道 `_query_fusion` 用 `uncertainty_predictor(query_feats, …)` 预测置信度，而论文 Eq.12 要求 ACM 只读 Δp。**致命**：论文第二个核心创新在跑的代码里并不存在；`disp_norm` 算了却从未使用 | 新增 `DisplacementConfidenceHead`，主管道改读 Δp；查询特征只交给 floor ψ |
| 2 | DGC 输出第 4 个自由度 `exp(s)` 沿视线缩放，论文 Eq.4 与 Theorem 1 中都没有 `s`。**致命**：ray 项依赖 P 本身且随距离无界增长，外推界失效 | 输出改回 R³，`P' = P + ΔP(h)`；`drone_pos` 不再参与计算 |
| 3 | σ 头吃 (Δp, P′, h)，论文 Eq.10 要求吃 (fᵃᵉʳ, rᵢ)；且加法形式 `σ_min + dev·h_ratio` 应为乘法 | 输入改为 `[f^aer; r_i]`，输出改为 `σ_0·(h/h_ref)·softplus(·)`；新增 rᵢ（到天底点的水平距离）计算 |
| 4 | 匹配代价是 `d/σ` 再乘置信度 + 贪心最近邻；论文 Eq.9 是 `‖ΔP‖²/(σ²+σ_floor²) + λ_sem(1−S_sem)` 并用 Hungarian | 三处匹配统一为 Eq.9 形式，改用 `linear_sum_assignment`，补上 σ_floor = 0.5m |
| 5 | τ 初值 `log(0.1)`（锐化，与论文「flatten，τ≈2–4」相反），且两个文件不一致；可学习 floor ε̂ 完全未实现 | τ 统一初始化为 1.0；实现 ε̂ = σ(ψ([Δp; F_d]))，返回 c̃ = (1−ε̂)·c |
| 6 | PE 频率 `10000^{+2i/(d/4)}` 符号与分母皆错、且 d/2 个分量自我重复；论文 Eq.5 要求 d 个独立分量 | 改为 `10000^{-2i/d}`、d/2 个频率，去掉 `cat([pe, pe])` |

顺带修掉一个使 CAA 完全失效的 bug：`sigma_v = 1.0` 作为余弦相似度阈值不可达，
导致 `sem_filter` 恒为 False、代价矩阵整体被置 1e6、Hungarian 的配对结果被全部丢弃。

论文侧（私有仓库）：

- Eq.6 补上 `(1−c)`，与 Algorithm 1 第 9 行一致，并说明 α(1−c)+(1−α)≤1 带来的幅值
  效应；同时修正「Eq.6 中的 (1−c) 因子」这一引用——原 Eq.6 并不含 (1−c)。
- ε̂ 的输入描述改为「在 ACM 的 Δp 基础上追加 F_d」，消除「与 ACM 输入相同」的错误说法。
- 外推数字（0.12m / 8.91m / 0.11–0.26m）与 abstract 的 0.26m、8.91m、6.4ms/8.7Hz
  标注为 `[TBD]`，避免以未实测数据的确定口吻陈述。

验证：冒烟测试确认 σ(50)/σ(25) = 2.000000（Eq.10 的乘法性）、ΔP ∈ R³ 且
P′ = P + ΔP、PE 为 64 个独立分量且频率自 1.0 递减至 1.33e-4、τ = 1.0、
c̃ ∈ (0,1) 且 1−c̃ ≥ ε̂、不传特征时 ε̂ = 0、NLL 有限。
